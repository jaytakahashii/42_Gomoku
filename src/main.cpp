...
public:
    ...
    GomokuGame() : window(sf::VideoMode({WINDOW_WIDTH, WINDOW_HEIGHT}), "42 Gomoku AI") {
        ...
        for(int i = 0; i < BOARD_SIZE; ++i)
            for(int j = 0; j < BOARD_SIZE; ++j) board[i][j] = Player::None;
    }
...
private:
    ...
    void render() {
        ...
        for (unsigned int i = 0; i < BOARD_SIZE; ++i) {
            ...
            sf::Vertex v_line[] = { sf::Vertex{{pos, OFFSET}}, sf::Vertex{{pos, (float)WINDOW_WIDTH - OFFSET}} };
            window.draw(v_line, 2, sf::PrimitiveType::Lines);
            sf::Vertex h_line[] = { sf::Vertex{{OFFSET, pos}}, sf::Vertex{{(float)WINDOW_WIDTH - OFFSET, pos}} };
